<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    <label class='control-label'>{{$form['label']}}
        ...
        @if($value)
            ...
            ?>
            <p><a data-lightbox='roadtrip' href='{{$url}}'><img style='max-width:180px' title="Image For {{$form['label']}}" src='{{$url}}'/></a></p>
            <?php else:?>
        ...
        @if(!$value)
            <div class="custom-file">
                <input type='file' id="{{$name}}" title="{{$form['label']}}" {{$required}} {{$readonly}} {{$disabled}} class='custom-file-input form-control' name="{{$name}}">
                <label class="custom-file-label" for="customFile">{{ @$form['help'] }}</label>
            </div>
        @else
